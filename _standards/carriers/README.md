...
| `locator_url_template` | yes (use `null` if none) | A Python `.format()` string like `"{base_url}/chap{locator}.html"`, used to build a link *if* the platform has public, stable per-unit URLs. Most third-party/login-gated platforms should just use `null` — do not guess a URL scheme you haven't confirmed. |
| `locator_titles` | optional | `{"<locator>": "<human title>"}` — e.g. `{"3": "Functions"}`. Shown next to the locator in tooltips/links: "Chapter 3 (Functions)". Skip it if you don't know unit/chapter titles. |
